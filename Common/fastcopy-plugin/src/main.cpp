...
// --- ТЕХНОЛОГИЯ ФИЛЬТРАЦИИ И ВАЛИДАЦИИ ФАЙЛОВ ПО МАСКАМ FAR 3 SDK ---
bool IsFileAllowedByFarFilter(HANDLE hFilter, const std::wstring& fileName, uint32_t attributes, uint64_t fileSize) {
    ...
    std::memset(&item, 0, sizeof(PluginPanelItem));

    // Делаем копию строки, так как SDK FAR требует неконстантный wchar_t*
    ...
    // Проверка элемента в созданном фильтре осуществляется через FileFilterControl
...
bool IsNetworkPathStr(const std::wstring& path) {
    if (path.length() >= 2 && path == L'\\' && path == L'\\') return true; // Windows UNC
...
// Автогенерация файлов локализации и смарт-макросов из внутренних строк бинарника
...
// --- ОБЯЗАТЕЛЬНЫЕ ДЕКЛАРАЦИИ ЭКСПОРТА ИНТЕРФЕЙСА FAR MANAGER 3 ---

// КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Без этой функции FAR 3 молча выгружает плагин из памяти!
DLL_EXPORT intptr_t WINAPI GetMinFarVersionW() {
    // Возвращаем минимальную поддерживаемую версию ядра (FAR Manager 3.0.0)
    return (3 << 24) | (0 << 16) | 0;
}

DLL_EXPORT void WINAPI SetStartupInfoW(const PluginStartupInfo *pInfo) { 
    // Защита: Если размеры структур не совпадают, плагин не должен падать
    if (pInfo && pInfo->StructSize >= sizeof(PluginStartupInfo)) {
        Info = *pInfo; 
        FSF = *pInfo->FSF; 
        DeployResourcesOnDemand(); 
    }
}

DLL_EXPORT void WINAPI GetPluginInfoW(PluginInfo *pInfo) { 
    if (pInfo) {
        // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Обнуляем структуру и выставляем ее точный размер для FAR 3
        std::memset(pInfo, 0, sizeof(PluginInfo));
...
// Нативная отправка строки макроса в ядро FAR 3
...
// Нативная сигнатура ProcessSynchroEventW для строгого FAR 3 SDK
...
void HandleGlobalHotkey(TaskType type) {
    ...
    PanelInfo passivePanel{}; 
    passivePanel.StructSize = sizeof(PanelInfo);
    ...
    // ИСПРАВЛЕНО: Вторым аргументом передается команда FCTL_GETPANELDIRECTORY, а не размер структуры
    Info.PanelControl(PANEL_PASSIVE, FCTL_GETPANELDIRECTORY, static_cast<intptr_t>(dirSize), fpd);
    b.destDirectory = fpd->Name;

    PanelInfo activePanel{}; 
    activePanel.StructSize = sizeof(PanelInfo);
    ...
    for (size_t i = 0; i < activePanel.SelectedItemsNumber; ++i) {
        ...
        std::memset(&gpi, 0, sizeof(FarGetPluginPanelItem));
...
// Нативный перехват ввода с консольных панелей
...
// Парсинг путей и галочек из штатных UI диалогов F5/F6
...
DLL_EXPORT HANDLE WINAPI OpenW(const OpenInfo *pInfo) {
    if (pInfo && pInfo->OpenFrom == OPEN_FROMMACRO) {
        ...
        if (omi && omi->Count > 0 && omi->Values.Type == FMVT_STRING) {
            std::wstring cmd = omi->Values.String;
            if (cmd == L"add_task_batch" && omi->Count >= 3) {
                FastCopyBatchOptions customBatch;
                customBatch.type = TaskType::Copy;
                if (omi->Values.Type == FMVT_STRING) {
                    customBatch.destDirectory = omi->Values.String;
                }

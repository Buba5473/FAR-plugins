...
// Вспомогательная функция закрытия хэндла потока (для GCC объявлена вверху)
...
void DiscoverDirectoryRecursive(const std::wstring& srcDir, const std::wstring& relPath, FastCopyBatchOptions& b) {
    ...
    do {
        // ИСПРАВЛЕНО: Безопасное прямое конструирование, исключающее ворнинг [-Waddress] в GCC 16
...
// --- ПЕРЕНОС АЛЬТЕРНАТИВНЫХ ПОТОКОВ ДАННЫХ (NTFS ADS) ---
void CopyAlternateDataStreams(const std::wstring& src, const std::wstring& dest) {
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            // ИСПРАВЛЕНО: Прямое конструирование, исключающее ворнинг [-Waddress] в GCC 16
...
// Колбэк-процедура для нативного асинхронного обновления прогресс-бара FAR + Inline Hashing
COPYFILE2_MESSAGE_ACTION CALLBACK Copy2ProgressRoutine(const COPYFILE2_MESSAGE* pMessage, PVOID pCallbackContext) {
    ...
    if (pMessage->Type == COPYFILE2_CALLBACK_STREAM_STARTED || pMessage->Type == 1) {
        ...
        // Интеграция Inline Hashing (Инкрементальный расчет XXH3 "на лету" из буферов ядра ОС)
